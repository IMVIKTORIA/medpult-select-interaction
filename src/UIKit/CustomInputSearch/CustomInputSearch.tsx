...
function CustomInputSearch<DataType>(props: CustomInputSearchProps<DataType>) {
  const { value, setValue, optionClickHandler, getDataHandler, ...restProps } =
    props;
  ...
  const handleOptionClick = (value: string, data?: DataType) => {
    ...
    if (optionClickHandler)
      return optionClickHandler({ value, data, closeCallback });
  ...
  return (
    <div className="custom-select" ref={rootRef}>
      <CustomInput
        ...
        isOpen={isOpen}
        buttons={[
          <InputButton svg={icons.Triangle} clickHandler={clickHandler} />,
        ]}
      />
      {isOpen && (
        <CustomSelectList
        ...
        >
          {values.map((item) => (
            <CustomSelectRow
              value={item.value}
              clickHandler={() =>
                handleOptionClick(item.value, item.code as any)
              }
            />
          ))}
        </CustomSelectList>
