...
interface FilterItemSearchProps
...
export default function FilterItemSearch(props: FilterItemSearchProps) {
  ...
  const deleteHandler = (code: string) => {
    filterValue.values = filterValue.values.filter(
      (item) => item.code !== code
    );
    setFilterValue(filterValue);
  };
  ...
  const getDataFiltered = async (
    page: number,
    query?: string
  ): Promise<FetchInputData> => {
    const data = await getDataHandler(page, query);
    data.items = data.items.filter(
      (value) =>
        !Boolean(filterValue.values.find((item) => item.code === value.code))
    );
  ...
  return (
    <FilterItemWrapper {...props}>
      <CustomInputSearch
        data={""}
        setValue={addValue}
        getDataHandler={getDataFiltered}
        isViewMode={false}
        value={selectedItemsLabel}
      />
      <div className="filter-item-search-variants">
        {filterValue.values.map((item) => (
          <FilterItemSearchElement
            name={item.value}
            deleteHandler={() => deleteHandler(item.code)}
          />
        ))}
      </div>
    </FilterItemWrapper>
